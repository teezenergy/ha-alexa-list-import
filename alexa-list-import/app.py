...
def login_mobile():
    ...
    dbg("login page status " + str(r.status_code))

    soup = BeautifulSoup(r.text, "html.parser")
    form = soup.find("form")
    if not form:
        print("[B] ERROR: No login form found (Amazon changed layout)")
        dbg("HTML snippet: " + r.text[:300])
        return False
    ...
    if "verification" in r3.url.lower():
